...
`cas_bai_sead.lua:142-143` keeps a 200 km influence radius with this comment:
...
- `frontline.lua` replaced the same threshold with parameter-free **Gabriel-graph base adjacency**,
  and its header records that spec 06 SECTOR-F16 **refuted** the fixed-km threshold as an EECH constant.
- `cas_bai_sead.lua:26` itself notes the invented `is_near_friendly` 200 km band was already replaced.

Also audit `troop.lua:103` (`SECTOR_RATIO_RADIUS = 200000`) — same smell, same origin, same pass.
...
this port — the convention `croute.lua` and `campaign_state.route_difficulty` document, `task.c:876`).
...
- [ ] Replace the metric radius with a topological rule, following the `frontline.lua` precedent
- [ ] Audit `troop.lua:103` in the same pass — realign or cite it
