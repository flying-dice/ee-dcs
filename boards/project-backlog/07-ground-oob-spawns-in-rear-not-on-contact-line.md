...
|---|---|---|
| PRIMARY | `ground_forces.ts:233` — friendly base centre +/- 800 m | boundary node, adjacent to enemy territory |
| ARTILLERY | `ground_forces.ts:302-304` — 1.5 km **into its own rear** | 2 hops behind the boundary node |
| SECONDARY | `ground_forces.ts:362-364` — 4 km **into its own rear** (`SEC_REAR_OFFSET`) | 1 hop behind the boundary node |

The objective is the nearest enemy base (`ground_forces.ts:437`), and the group drives there at
`COL_SPEED = 8` m/s (`ground_forces.ts:37`).
...
This is not a one-off at OOB. Reinforcement re-spawns at the base too (`ground_forces.ts:626`), so
...
**base** (`ground_forces.ts:596-604`), never behind the primary group — so it holds ~47-99 km
behind the fighting instead of EECH's one road-hop.

## Secondary finding — two divergent frontline definitions

`ground_forces.ts` does **not** require `frontline.ts`. It defines its own `frontline_bases()`
(`ground_forces.ts:137-155`) using a 140 km metric band (`config.ts:234 front_dist`). That is
precisely the invented fixed-km threshold that `frontline.ts:20` records as **REFUTED** by spec 06
SECTOR-F16 and replaced with parameter-free Gabriel-graph adjacency. So the air war runs on the
corrected definition while the ground war still runs on the refuted one. `installations.ts:102`
shares the same band.

## Third finding - COL_SPEED is uncited and too slow

`ground_forces.ts:37` sets `COL_SPEED = 8` m/s with no citation. EECH's ground vehicles cruise at
...
Bases are the port's sector/node proxy (the convention `croute.ts` and
...
`frontline.ts`'s adjacency so there is one frontline definition, and derive the echelon offsets from
...
- [ ] Replace `ground_forces.frontline_bases()` with `frontline.ts` Gabriel adjacency; retire the duplicate `front_dist` band
...
- [ ] Fix reinforcement (`ground_forces.ts:626`) to use the same placement, not the base
...
- [ ] Realign `COL_SPEED` (`ground_forces.ts:37`) to the cited `vh_dbase.c` cruise velocity; prefer per-member slowest-in-group (`croute.c:316`) over one constant
...
- **claude** (2026-09-21T16:52:00.000Z): Followed up the one item I had flagged as unaudited in the original write-up. `COL_SPEED` at `ground_forces.ts:37` traces to nothing; EECH's value is `vh_dbase.c:114` (20 kt = 10.29 m/s), uniform across 25 of 38 vehicle rows. Folded into this card rather than raised separately - it is the same defect surface (ground movement) and fixing placement without speed would leave the symptom half-corrected.
