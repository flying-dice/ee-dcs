...
`imap.ts:36-37` uses flat, uncited radii and its header claims they are "not per-keysite in our
approximation". That is false — they are per-sub-type literals in the EECH keysite_database.

| | port (`imap.ts:36-37`) | EECH (`ks_dbase.c`) |
|---|---|---|
...
`installations.ts:72` already transcribes each kind's `ks_dbase.c` row, but only the **boolean**
columns. Extend it with the numeric ones and have `imap.ts` read per-kind.
...
- [ ] Extend `installations.ts:72` FLAGS rows with `importance`, `importance_radius`, `air_coverage_radius`, `recon_distance`
- [ ] Expose via `installations.flags_for_kind`; cover the `airbase` / `farp` kinds set by `farps.ts`
- [ ] Rewrite `imap.ts` to read per-kind — a kind with air_coverage_radius 0 must contribute NOTHING, not a default
- [ ] Delete the false "not per-keysite" claim from the `imap.ts` header; cite `ks_dbase.c` / `imaps.c`
- [ ] Fix the stale `keysite.ts:269` claim ("all port keysites are airbases, equal importance 1.0")
